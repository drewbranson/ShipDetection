...
for F in $(ls -1 "${sourceDirectory}"/S1*.zip); do
  ...
  echo "Executing: "${gptPath} ${graphXmlPath} -q 4 -c 26G -x -Pfile=${sourceFile} -Ptarget=${targetFile} 
  ${gptPath} ${graphXmlPath} -q 4 -c 26G -x -Pfile=${sourceFile} -Ptarget=${targetFile} 
